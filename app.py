# ==============================================================
# 📊 Indicadores Financeiros - Dashboard Streamlit
# ==============================================================
# Instale as dependências no Colab (se necessário):
# !pip install streamlit pandas plotly numpy openpyxl yfinance
...
# ==============================
# FUNÇÕES DE VALUATION E COTAÇÃO
# ==============================
...
if modo_analise == "🏆 Dados Gerais":
    ...
    with rank_tab1:
        ...
        if not rentabilidade_consolidado.empty:
            rentabilidade_formatado = formatar_dataframe_percentual(rentabilidade_consolidado, ['ROE', 'ROA', 'ROI', 'Margem Líquida'])
                ...
                with tab2:
                    if is_bank:
                        ...
                        if resultado_abrangente and pl_medio and numero_acoes and numero_acoes > 0:
                            ...
                            # CORREÇÃO AQUI: bloco st.info com f-string bem formatada
                            st.info(
                                f"""
                                **📊 Fórmula de Valuation para Bancos (Vellani, 2024):**
                                LPA = Resultado Abrangente ÷ Nº de Ações = {formatar_moeda_brasil_correta(resultado_abrangente)} ÷ {formatar_numero_brasil_correto(numero_acoes, 0)} = R$ {lpa:,.2f}
                                VPA = PL Médio ÷ Nº de Ações = {formatar_moeda_brasil_correta(pl_medio)} ÷ {formatar_numero_brasil_correto(numero_acoes, 0)} = R$ {vpa:,.2f}
                        ...
                        if ebitda_valor is not None and resultado_operacional is not None:
                            ...
                            if lucro_economico_valor is not None and lucro_economico_valor > 0:
                                ...
                                if valor_empresa:
                                    ...
                                    dados_cotacao = buscar_cotacao_atual(ticker_selecionado)
                                    # Cálculo da SELIC implícita e EBITDA necessário
                                    ...
                                    if ebitda_necessario is not None and ebitda_necessario > 0:
                                        ebitda_atual = df_filtrado["EBITDA"].iloc[0] if pd.notna(df_filtrado["EBITDA"].iloc[0]) else 0
                                        variacao_necessaria = ((ebitda_necessario - ebitda_atual) / ebitda_atual) * 100 if ebitda_atual != 0 else 0
