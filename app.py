...
# ==============================
    ...
    if not df_empresa_todos_anos.empty:
        ...
        tab_atual, tab_evolucao, tab_div, tab_sim = st.tabs(["📊 Análise do Ano", "📈 Evolução Temporal", "💰 Dividendos", "💵 Simulação"])
        ...
        with tab_div:
            ...
            df_div = buscar_dividendos_historicos(ticker_selecionado)
            if df_div is not None:
                ...
                col3.metric("Último Provento", f"R$ {stats['ultimo_dividendo']:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."), help=f"Data: {stats['data_ultimo'].strftime('%d/%m/%Y') if stats['data_ultimo'] else 'N/A'}")
                cotacao_div = buscar_cotacao_atual(ticker_selecionado)
                dy = None
                if cotacao_div and cotacao_div['cotacao'] > 0:
                    data_limite = datetime.now() - timedelta(days=365)
                    div_12m = df_div[df_div['Data'] >= data_limite]
                    if not div_12m.empty:
                        total_12m = div_12m['Dividendo'].sum()
                        dy = (total_12m / cotacao_div['cotacao']) * 100
                col4.metric("Dividend Yield (12M)", formatar_percentual_brasil(dy/100) if dy is not None else "N/A")
                st.markdown("---")
                df_anual = df_div.groupby('Ano')['Dividendo'].sum().reset_index()
                df_anual.columns = ['Ano', 'Total']
                fig = px.bar(df_anual, x='Ano', y='Total', title=f"Proventos por Ano - {ticker_selecionado}")
                fig.update_layout(yaxis_tickformat=',.2f')
                st.plotly_chart(fig, use_container_width=True)
                st.subheader("Histórico")
                display = df_div[['Data', 'Dividendo']].copy()
                display.columns = ['Data (Ex)', 'Valor (R$)']
                display['Valor (R$)'] = display['Valor (R$)'].apply(lambda x: f"R$ {x:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
                ...
                st.warning("Não foi possível buscar dados de dividendos para este ticker.")
